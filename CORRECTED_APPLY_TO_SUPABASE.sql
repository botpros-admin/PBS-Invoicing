-- ============================================================================
-- CORRECTED SQL MIGRATIONS FOR LABORATORY BILLING FEATURES
...
-- ============================================================================
-- 2. ADD PARENT_CLIENT_ID TO CLIENTS TABLE (if not exists as parent_id)
-- ============================================================================
-- The clients table already has parent_id field, so we'll use that
-- Just ensure it references the correct table
ALTER TABLE public.clients 
DROP CONSTRAINT IF EXISTS clients_parent_id_fkey;

ALTER TABLE public.clients
ADD CONSTRAINT clients_parent_id_fkey 
FOREIGN KEY (parent_id) REFERENCES public.clients(id) ON DELETE SET NULL;
...
-- ============================================================================
-- 5. PARENT ACCOUNT AGGREGATION FUNCTIONS (Updated to use parent_id)
-- ============================================================================
...
BEGIN
  ...
  WITH parent_stats AS (
    ...
    WHERE p.organization_id = org_id
      AND p.parent_id IS NULL  -- Only get parent accounts
...
BEGIN
  ...
  WITH monthly_data AS (
    ...
    WHERE (c.id = parent_id_param OR c.parent_id = parent_id_param)
      AND i.organization_id = org_id
...
-- ============================================================================
-- 6. RLS POLICIES FOR NEW TABLES (Only if current_user_org_id exists)
-- ============================================================================

-- First check if the helper function exists, if not create a simple one
...
-- UNION ALL
-- SELECT 
--     'clients parent_id field' as feature,
--     EXISTS(SELECT 1 FROM information_schema.columns 
--            WHERE table_name = 'clients' 
--            AND column_name = 'parent_id') as exists
-- UNION ALL
-- SELECT 
