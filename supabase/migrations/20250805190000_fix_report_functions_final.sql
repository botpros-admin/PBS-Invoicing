-- Drop existing functions to ensure a clean update
DROP FUNCTION IF EXISTS get_client_performance(text, text);
DROP FUNCTION IF EXISTS get_client_performance_by_range(text, text);
DROP FUNCTION IF EXISTS get_top_cpt_codes(text, text, integer);

-- Recreate get_client_performance_by_range with explicit type casting to match return types
...
-- Recreate get_top_cpt_codes with the correct JOIN and column reference
...
BEGIN
    ...
    SELECT 
        ...
        SUM(ii.total)::NUMERIC as value
    FROM invoice_line_items ii
    JOIN invoices i ON ii.invoice_id = i.id
    JOIN cpt_codes c ON ii.cpt_code_id = c.id -- Correctly join to get the code and description
